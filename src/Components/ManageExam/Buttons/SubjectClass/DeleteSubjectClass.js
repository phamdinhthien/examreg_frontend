...
class DeleteSubjectClass extends Component {
    constructor(props) {
        super(props);
        this.state = {
            modal: false,
    ...
    onDeleteSubjectClass = () => {
        ...
        fetch(ApiConfig.API_URL + '/SubjectClasses/DeleteOneSubjectClass.php?id=' + this.props.subjectClasseID)
            .then(res => res.json())
            .then(response => {
                alertTextCustom("Xóa lớp học phần thành công", "#28a745");
                this.props.loadData();
                this.setState({
                    modal: false
                });
            })
            .catch(err => alertText('Xóa lớp học phần không thành công'));
    }
            ...
            <div className="EditRemove">
                <div>
                    ...
                    <Modal isOpen={modal} toggle={this.toggle}>
                        ...
                        <ModalBody>
                            Bạn Có Muốn Lớp Học Phần Này
                    </ModalBody>
                        <ModalFooter>
                            {!this.state.loading
                                ?
                                <div>
                                    <Button color="danger" onClick={this.onDeleteSubjectClass}>Xóa</Button>{' '}
...
export default DeleteSubjectClass;